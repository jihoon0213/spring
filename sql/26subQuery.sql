...
SELECT (SELECT SUM(od.Quantity * p.Price)
       ...
       (SELECT COUNT(DISTINCT EmployeeID)
        FROM w3schools.Orders o
        WHERE o.OrderDate BETWEEN '1996-09-01' AND '1996-09-30') price;
...
# 연습
# 1996-08 월 평균 처리 금액 보다 높은 금액을 처리한 직원 목록
# 1. 총 처리 금액
# 2. 직원 수 나누고
# 3. 직원별 처리 금액
# 4. 평균금액보다 높은 직원 조회
...
SELECT (SELECT SUM(p.Price * od.Quantity)
        FROM w3schools.Orders o
                 JOIN w3schools.OrderDetails od
                      ON o.OrderID = od.OrderID
                 JOIN w3schools.Products p
       ...
       (SELECT COUNT(DISTINCT EmployeeID)
        FROM w3schools.Orders o
        WHERE o.OrderDate BETWEEN '1996-08-01' AND '1996-08-31') price;

SELECT e.EmployeeID, e.FirstName, e.LastName, SUM(p.Price * d.Quantity) sum
FROM w3schools.Orders o
...
WHERE o.OrderDate BETWEEN '1996-08-01' AND '1996-08-31'
GROUP BY e.EmployeeID
HAVING sum > (SELECT SUM(p.Price * od.Quantity)
              FROM w3schools.Orders o
                       JOIN w3schools.OrderDetails od
                            ON o.OrderID = od.OrderID
                       JOIN w3schools.Products p
